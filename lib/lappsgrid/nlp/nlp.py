...
def getLabel(annotation):
    type = annotation['@type']
    if type == 'http://vocab.lappsgrid.org/NamedEntity':
        return annotation['features']['category']
    if type == 'http://vocab.lappsgrid.org/Token':
...
def brat(lappsJson):
    ...
    entities = [ createEntity(a) for a in annotations if a['@type'] == Uri.TOKEN ]
    ne = [ createEntity(a) for a in annotations if a['@type'] == Uri.NE ]
    docData['entities'] = entities + ne

    dependency_parse = processDependencies(annotations)
    constituency_parse = processPhraseStructure(annotations)
    docData['relations'] = dependency_parse + constituency_parse

    collData = dict()
    collData['entity_types'] = types.entity_types
